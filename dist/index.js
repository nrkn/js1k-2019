        ...
        while (roomCount--) {
            ...
            for (let y = 0; y < height; y++) {
                for (let x = 0; x < width; x++) {
                    mapData[key(x + current[0] - ~~(width / 2), y + current[1] - ~~(height / 2))] = floor + ~~(Math.random() * 2);
                    if ((x + current[0] - ~~(width / 2)) !== player[0] &&
                        !~~(Math.random() * monsterChance)) {
                        monsters[monsters.length] = [
                            x + current[0] - ~~(width / 2),
                            y + current[1] - ~~(height / 2)
                        ];
                        mapData[key(x + current[0] - ~~(width / 2), y + current[1] - ~~(height / 2))] = monster;
                    }
                }
            }
        }
